...
def main():
    ...
    ap.add_argument("--artifact_path", type=str,
                    default="./pd_hc_binary/best_pipeline_auc_RandomForest.joblib",
...
# 사람/손 단위 판정 임계
    ap.add_argument("--thresh", type=float, default=0.50, help="PD 판정 임계값")
        ...
        for col in expected_prefixed_cols:
            if col.startswith("num__"):
                ...
                if "_" in base:
                    ...
                    val = df_norm.get(feat, "unknown").astype(str).fillna("unknown")
    ...
    try:
        ...
        if isinstance(bundle, Pipeline):
            ...
            person_thresh = bundle.get("person_threshold", args.thresh)  # <<< 학습단계에서 저장된 임계값
        else:
            model = bundle
        ...
        # ---------- 모델 + feature_names_in_ ----------
            ...
            raise RuntimeError("아티팩트를 해석할 수 없습니다.")
        ...
        # --- 사람 단위(Noisy-OR) ---
        probs = df_out["probability"].values
        p_noisy_or = 1 - np.prod(1 - probs)   # Noisy-OR 결합
        final_pred = int(p_noisy_or >= person_thresh)

        zone = "yellow" if (person_thresh - args.delta) <= p_noisy_or < (person_thresh + args.delta) \
               else ("red" if final_pred == 1 else "green")

        trigger_hand = df_out.loc[df_out["probability"].idxmax(), "hand"]

        print(f"[PERSON-NOISYOR] {features_fname}: "
              f"{'환자(PD)' if final_pred==1 else '정상(HC)'} "
              f"(결합위험도={p_noisy_or:.2f}, 트리거손={trigger_hand}, zone={zone})")

        per_person = pd.DataFrame([{
            "filename": features_fname,
            "noisyor_prob": p_noisy_or,
            "final_pred": final_pred,
            "trigger_hand": trigger_hand,
            "zone": zone
        }])

        person_path = pred_path.replace("predictions_", "person_level_noisyor_")
