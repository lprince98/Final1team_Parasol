...
def main():
    ...
    ap.add_argument("--thresh", type=float, default=0.50,
                    help="사람/손 단위 PD 판정 임계값(기본 0.50)")
    ...
    # --------- 유틸 ---------
    def find_column_transformer(pipe):
    ...
    try:
        ...
        # --- 방금 저장한 features_*.csv 로드 ---
        df_raw_all = pd.read_csv(out_path)
        ...
        if pipeline is not None:
            ...
            y_prob = pipeline.predict_proba(df_infer)[:, 1] if hasattr(pipeline, "predict_proba") else np.full(len(df_infer), np.nan)
            y_pred = pipeline.predict(df_infer) if hasattr(pipeline, "predict") else (y_prob >= args.thresh).astype(int)

        # ---------- 분기 2: 모델 + selected_features(접두사) ----------
            ...
            y_prob = model.predict_proba(df_pref)[:, 1] if hasattr(model, "predict_proba") else np.full(len(df_pref), np.nan)
            y_pred = model.predict(df_pref) if hasattr(model, "predict") else (y_prob >= args.thresh).astype(int)

        # ---------- 분기 3: 모델 + feature_names_in_ ----------
            ...
            raise RuntimeError("아티팩트를 해석할 수 없습니다. (pipeline/model/selected_features/feature_names_in_ 중 하나 필요)")

        # --- 손 단위 결과: 임계값 적용 + 저장/로그 ---
        df_out = df_raw_all.copy()
        df_out["probability"] = y_prob
        df_out["prediction_raw"] = y_pred
        df_out["prediction"] = (df_out["probability"] >= args.thresh).astype(int)

        for _, row in df_out.iterrows():
            label = "환자(PD)" if row["prediction"] == 1 else "정상(HC)"
            print(f"[RESULT] {row.get('hand','?')}: {label} (위험도={row['probability']:.2f}, TH={args.thresh:.2f})")
        ...
        # --- 사람 단위 OR 규칙: 둘 중 하나라도 PD면 PD ---
        per_person = (df_out.groupby("filename", as_index=False)
                             .agg(final_prob=("probability", "max")))
        per_person["final_pred"] = (per_person["final_prob"] >= args.thresh).astype(int)

        # 어떤 손이 트리거였는지도 기록
        idx_max = df_out.groupby("filename")["probability"].idxmax()
        trigger = df_out.loc[idx_max, ["filename", "hand", "probability"]].rename(
            columns={"hand": "trigger_hand", "probability": "trigger_prob"}
        )
        per_person = per_person.merge(trigger, on="filename", how="left")

        # 옐로존(재측정 권고)
        delta = getattr(args, "delta", 0.05)
        per_person["zone"] = np.where(
            (per_person["final_prob"] >= args.thresh - delta) & (per_person["final_prob"] < args.thresh + delta),
            "yellow",
            np.where(per_person["final_pred"] == 1, "red", "green")
        )

        person_path = pred_path.replace("predictions_", "person_level_")
        per_person.to_csv(person_path, index=False)

        for _, r in per_person.iterrows():
            lbl = "환자(PD)" if r["final_pred"] == 1 else "정상(HC)"
            print(f"[PERSON] {r['filename']}: {lbl} (최대위험도={r['final_prob']:.2f}, 손={r['trigger_hand']}, zone={r['zone']})")
        print(f"[INFO] Saved person-level decisions to {person_path}")
